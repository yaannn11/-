...
class EvolutionarySamplingAgent:
    ...
    def __init__(self, obj_func, dim, lb, ub, max_nfe=1000):
        ...
        self.agent = QLearningAgent(num_actions=4, num_states=72)
        self.current_state = np.random.randint(72)
        self.history_nfe = []
    ...
    def run(self):
        ...
        while self.nfe < self.max_nfe:
            ...
            # 判斷是否改善
            improved = self.best_y < best_y_before
            improvement = best_y_before - self.best_y
            reward = improvement / (abs(best_y_before)+1e-12)
            reward = np.clip(reward,-1,1)

            # 更新 Improvement History
            if improved:
                self.improve_history.append(1)
            else:
                self.improve_history.append(0)
  
            # 計算 improvement rate
            if len(self.improve_history) > 0:
                improvement_rate = np.mean(self.improve_history)
            else:
                improvement_rate = 0.0
            # Adaptive Temperature
            self.agent.update_temperature(improvement_rate)

            # 將 improvement rate 離散化
            if improvement_rate > 0.5:
                improve_state = 2       # 改善良好

            elif improvement_rate > 0:
                improve_state = 1       # 有小改善

            else:
                improve_state = 0       # 沒改善 
            
            progress = self.nfe / self.max_nfe
            if progress < 0.33:
                stage_state = 0
                ...
                stage_state = 2
            success_state = 1 if reward > 0.5 else 0


            next_state = (action * 18+ stage_state * 6+ improve_state * 2+ success_state)
            self.agent.update(state, action, reward, next_state)
            state = next_state
            


            next_state = (action * 6+ improve_state * 2+ success_state)
            self.agent.update(state, action, reward, next_state)
            state = next_state
