...
def save_prediction_errors(results_dict, functions, seeds, base_dir):
    ...
    with open(rmse_path, 'w', encoding='utf-8') as f_rmse:
        ...
        for f in functions:
            for s in seeds:
                ...
                for r in records:
                    f_rmse.write(f"{f},{s},{r['nfe']},{r['action']},{r['state']},{r['reward']:.1f},"f"{r['predicted_y']:.6e},{r['actual_y']:.6e},{r['error']:.6e},"f"{r['relative_error']:.6e},{r['min_dist_db']:.6e}\n")
    print(f"Prediction errors file saved to {rmse_path}")
    
# 6.2 Save Condition Numbers
def save_condition_numbers(results_dict, functions, seeds, base_dir):
...
# 6.3 Save Q-tables
def save_q_tables(results_dict, functions, seeds, base_dir):
...
def save_strategy_usage(results_dict, functions, seeds, base_dir):
...
def export_all(results_dict, functions, seeds, base_dir):
    ...
    save_prediction_errors(results_dict, functions, seeds, base_dir)
    save_condition_numbers(results_dict, functions, seeds, base_dir)
    save_q_tables(results_dict, functions, seeds, base_dir)
    save_strategy_usage(results_dict, functions, seeds, base_dir)